...
`ronna-screen` 在 `3.5.0` 版本移除了 `cross-env`，采用 `pnpm` 中 [shell-emulator](https://pnpm.io/zh/cli/run#shell-emulator)这个配置项替代。如果您替换成了 `yarn` 或者 `npm` ，则需要安装 `cross-env`，还原此处的代码即可 [commit](https://github.com/ronnacesronna-admin-thin/commit/820f724d5bec14951cbe81d7c6984451de66d93b#diff-7ae45ad102eab3b6d7e7896acd08c427a9b25b346470d7bc6507b6481575d519)
...
- 如何解决？  
  答：如果想内容区的单文件支持多个根元素，可以来到这里 [appMain.vue](https://github.com/ronnaces/ronna-screen/blob/main/src/layout/components/appMain.vue#L41) 将 `Transition` 以及相关地方删除，不在要页面内容过渡效果了。
...
- 如何避免？  
  答：可以在 [此处](https://github.com/ronnaces/ronna-screen-doc/blob/main/public/platform-config.json#L25) 修改 `ResponsiveStorageNameSpace` 为您所需的命名空间即可
...
答：这里报错来自于 [vite-plugin-remove-console](https://github.com/xiaoxian521/vite-plugin-remove-console)，当 `console.log` 写法不规范或者这个文件是压缩格式时就会出现这种报错，请在 `external` 中排除这个 `console.log` 出现的文件，参考 [removeConsole](https://github.com/ronnaces/ronna-screen/blob/main/build/plugins.ts#L50)

## 安装依赖慢，如何解决？

可能您刚接触到 `ronna-screen` 会有个疑问，为什么平台使用了 `pnpm` 可是安装平台依赖的时候还是等了很长时间呢？
...
③ 当我们将本地源换成 `npmmirror` 后，再回到 `ronna-screen` 平台，将 `pnpm-lock.yaml` 和 `node_modules` 这两个文件删除，然后执行下面的命令重新安装依赖，您就会发现安装速度很快啦
...
当然第二种办法也是最方便的办法就是将下面代码加入到您自己库的 `package.json` 里 [查看具体参考](https://github.com/ronnacesronna-admin-table/blob/main/package.json#L24-L26)
...
答：可能是您引入某个库的问题，比如 [aws-sdk-js](https://github.com/aws/aws-sdk-js)，解决办法就是将 [src/utils/globalPolyfills.ts](https://github.com/ronnaces/ronna-screen-doc/blob/main/src/utils/globalPolyfills.ts) 引入 [src/main.ts](https://github.com/ronnaces/ronna-screen-doc/blob/main/src/main.ts)
...
答：在平台 `3.9.2` 版本，我把样式覆盖了，使其表现更鲜明（细心的小伙伴会发现 `vscode` 好多操作鼠标覆盖上去也是这种样式风格 🤗️），具体代码修改位置 [第一处](https://github.com/ronnaces/ronna-screen/commit/c80818d792276666aaea4b18413a0f08777f2ed1#diff-65a69f4a70b50de99604a7cae0784ee0e63a22f5d3addc384d50f76bbba1a2c2)、[第二处](https://github.com/ronnaces/ronna-screen/commit/c80818d792276666aaea4b18413a0f08777f2ed1#diff-6e577e4119a687e405cdbfac69a8fa06cbcc3b80eeca1d3766d936961de4f3a5)，如果想保持一致删除对应改动处即可
...
答：[请点击这里查看原因](https://github.com/ronnaces/ronna-screen/issues)
...
:::tip 具体修改记录
[点击查看完整版具体修改记录](https://github.com/ronnaces/ronna-screen-doc/commit/3629a66535d3d47bec04be9f73becbd12d339fe0)  
[点击查看非国际化精简版具体修改记录](https://github.com/ronnaces/ronna-screen-thin/commit/27056e75609637508fa8818439c553082600b00f)  
[点击查看国际化精简版具体修改记录](https://github.com/ronnaces/ronna-screen-thin/commit/23d9cdaf81cb543b8659d7c2b5f969e8b53364b5)
:::
...
答：[请点击这里查看原因](https://github.com/ronnaces/ronna-screen/issues/397#issue-1492326492)
...
3. 将 `lodash`、`lodash-es`、`lodash-unified` 这三个库加入到 [include](https://github.com/ronnaces/ronna-screen/blob/main/build/optimize.ts#L7)

4. 如果需要在生产环境使用 `lodash cdn` ，将下面代码加入到 [modules](https://github.com/ronnaces/ronna-screen/blob/main/build/cdn.ts#L12)
...
答：解决办法如下图，来自 [issues/953](https://github.com/ronnaces/ronna-screen/issues/953#issuecomment-1976024071)
