...
[.env.staging](https://github.com/ronnaces/ronna-screen/blob/main/.env.staging) 文件为预发布打包前的配置文件，[预发布](https://cn.vitejs.dev/guide/env-and-mode.html#modes) <Badge text="vite文档"/>
...
[.env.production](https://github.com/ronnaces/ronna-screen/blob/main/.env.production) 文件为正式环境打包前的配置文件
...
平台提供了全局打包路径 [VITE_PUBLIC_PATH](https://github.com/ronnaces/ronna-screen/blob/main/.env.production#L2) ，默认 `/`，可根据需求自行修改。比如平台的预览地址是 `https://github.com/ronnaces/ronna-screen/#/login`，可以看到根目录 `https://github.com/ronnaces` 后面又跟了个 `/ronna-screen/` 子目录，那么我们打包时就应该把 `VITE_PUBLIC_PATH` 改成 `/ronna-screen/`，然后执行 `pnpm build` 就行，打包完后观察平台根目录会多出个 `dist` 目录，如下图
...
:::tip 上面的配置是平台的 VITE_PUBLIC_PATH = / 情况，也就是只有跟目录的情况。下面的配置为有子目录情况，比如子目录叫 `ronna-screen`

① 来到 [.env.production](https://github.com/ronnaces/ronna-screen/blob/main/.env.production#L2)，将 `VITE_PUBLIC_PATH` 等于 `/ronna-screen`，如下
...
② 配置 `nginx.config` 如下，可以看到多了个 `location /ronna-screen/` 配置
...
① 来到 [.env.production](https://github.com/ronnaces/ronna-screen/blob/main/.env.production#L16)，将 `VITE_COMPRESSION` 等于 `both`，如下
